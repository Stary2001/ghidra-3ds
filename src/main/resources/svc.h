...
// adapted from libctru svc.h+types.h
...
enum ResourceLimitType {
	...
	RESLIMIT_BIT            = 1<<31,  ///< Forces enum size to be 32 bits
};
...
enum {
	DMACFG_SRC_IS_DEVICE        = 1<<0, ///< DMA source is a device/peripheral. Address will not auto-increment.
	DMACFG_DST_IS_DEVICE        = 1<<1, ///< DMA destination is a device/peripheral. Address will not auto-increment.
	DMACFG_WAIT_AVAILABLE       = 1<<2, ///< Make \ref svcStartInterProcessDma wait for the channel to be unlocked.
	DMACFG_KEEP_LOCKED          = 1<<3, ///< Keep the channel locked after the transfer. Required for \ref svcRestartDma.
	DMACFG_USE_SRC_CONFIG       = 1<<6, ///< Use the provided source device configuration even if the DMA source is not a device.
	DMACFG_USE_DST_CONFIG       = 1<<7, ///< Use the provided destination device configuration even if the DMA destination is not a device.
};

/// Configuration flags for \ref svcRestartDma.
enum {
	DMARST_UNLOCK           = 1<<0, ///< Unlock the channel after transfer.
	DMARST_RESUME_DEVICE    = 1<<1, ///< Replace DMAFLUSHP instructions by NOP (they may not be regenerated even if this flag is not set).
};
...
enum DebugFlags {
	DBG_INHIBIT_USER_CPU_EXCEPTION_HANDLERS   = 1<<0, ///< Inhibit user-defined CPU exception handlers (including watchpoints and breakpoints, regardless of any @ref svcKernelSetState call).
	DBG_SIGNAL_FAULT_EXCEPTION_EVENTS         = 1<<1, ///< Signal fault exception events. See @ref FaultExceptionEvent.
	DBG_SIGNAL_SCHEDULE_EVENTS                = 1<<2, ///< Signal schedule in/out events. See @ref ScheduleInOutEvent.
	DBG_SIGNAL_SYSCALL_EVENTS                 = 1<<3, ///< Signal syscall in/out events. See @ref SyscallInOutEvent.
	DBG_SIGNAL_MAP_EVENTS                     = 1<<4, ///< Signal map events. See @ref MapEvent.
} ;
...
enum ThreadContextControlFlags {
	THREADCONTEXT_CONTROL_CPU_GPRS  = 1<<0, ///< Control r0-r12.
	THREADCONTEXT_CONTROL_CPU_SPRS  = 1<<1, ///< Control sp, lr, pc, cpsr.
	THREADCONTEXT_CONTROL_FPU_GPRS  = 1<<2, ///< Control d0-d15 (or s0-s31).
	THREADCONTEXT_CONTROL_FPU_SPRS  = 1<<3, ///< Control fpscr, fpexc.

	THREADCONTEXT_CONTROL_CPU_REGS  = 1<<0 | 1<<1, ///< Control r0-r12, sp, lr, pc, cpsr.
	THREADCONTEXT_CONTROL_FPU_REGS  = 1<<2 | 1<<3, ///< Control d0-d15, fpscr, fpexc.

	THREADCONTEXT_CONTROL_ALL       = 1<<0 | 1<<1 | 1<<2 | 1<<3, ///< Control all of the above.
};
